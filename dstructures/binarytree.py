...
class BinaryTreeNode(object):
    ...
    def __init__(self, data):
        """Initializes this binary tree node with the given data"""
        self.data = data
        self.left = None
        self.right = None
    ...
    def is_leaf(self):
        ...
        return self.left is None and self.right is None # O(1)

    def is_branch(self):
        """
        Returns True if this node is a branch (has at least one child)
        """
        print(self.left is None, self.right is None)
        # Checks if either left child or right child has a value
        return self.left is not None or self.right is not None # O(1)

    def height(self):
        """
        Returns the height of this node (the number of edges on the longest
        ...
        left_height = 0
        if self.left: # O(1)
            left_height = self.left.height() + 1 # O(log(n))
        # Checks if right child has a value and if so calculate its height
        right_height = 0
        if self.right: # O(1)
            right_height = self.right.height() + 1 # O(log(n))
...
class BinarySearchTree(object):
    ...
    def height(self):
        """
        Returns the height of this tree (the number of edges on the longest
        downward path from this tree's root node to a descendant leaf node).
        
        Running time: O(n) - height function is O(n)
        """
    ...
    def _find_node_recursive(self, item, node):
        """
        Returns the node containing the given item in this binary search tree,
        ...
        # Check if the given item is less than the node's data
            ...
            return self._find_node_recursive(item, self.left)
